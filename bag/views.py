    ...
    if size:
        if item_id in list(bag.keys()):
            if size in bag[item_id]['size_of_item'].keys():
                ...
                messages.success(
                    request, f'Updated size {size.upper()} of\
                    {product.name} quantity to\
                    {bag[item_id]["size_of_item"][size]}')
            else:
                bag[item_id]['size_of_item'][size] = quantity
                messages.success(
                    request, f'Added size {size.upper()} of\
        ...
        if item_id in list(bag.keys()):
            ...
            messages.success(
                request, f'{product.name} quantity updated to\
                {bag[item_id]}')
        else:
            bag[item_id] = quantity
            messages.success(
                request, f'{product.name} successfully added\
                to your bag!')
