    ...
    if size:
        if item_id in list(bag.keys()):
            if size in bag[item_id]['size_of_item'].keys():
                ...
                messages.success(request, f'Updated size {size.upper()} of {product.name} quantity to {bag[item_id]["size_of_item"][size]}')
            else:
                bag[item_id]['size_of_item'][size] = quantity
...
def adjust_bag(request, item_id):
    ...
    if size:
        if quantity > 0:
            bag[item_id]['size_of_item'][size] = quantity
            messages.success(request, f'Updated size {size.upper()} {product.name} quantity to {bag[item_id]["size_of_item"][size]}')
        else:
...
def remove_from_bag(request, item_id):
    """A view to remove unwanted item from the bag"""
